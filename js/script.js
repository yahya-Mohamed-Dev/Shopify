...
(function () {
  ...
  /* ---------- 8. Hero video autoplay + safe fade-in (no black screen) ---------- */
  const heroVideo = document.querySelector('.hero-video');

  if (heroVideo) {
    // The image background sits permanently underneath, so even if this
    // video never plays the hero always has a solid, on-brand background.

    // Fade the video in ONLY once it actually begins playing frames.
    heroVideo.addEventListener('playing', () => heroVideo.classList.add('playing'));
    heroVideo.addEventListener('timeupdate', () => heroVideo.classList.add('playing'));

    // If anything goes wrong, keep the image visible (default state).
    heroVideo.addEventListener('error', () => heroVideo.classList.remove('playing'));
    heroVideo.addEventListener('stalled', () => heroVideo.classList.remove('playing'));

    try {
      const playPromise = heroVideo.play();
      if (playPromise) {
        playPromise.then(() => heroVideo.classList.add('playing')).catch(() => heroVideo.classList.remove('playing'));
      }
    } catch (e) {
